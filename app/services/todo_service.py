...
class TodoService:
    ...
    def create_todo(self, db: Session, todo: TodoCreate, user_id: int, background_tasks=None) -> Todo:
        ...
        if background_tasks:
            background_tasks.add_task(self.background_refine, saved_todo.id, user_id)
            
        return saved_todo

    def background_refine(self, todo_id: int, user_id: int):
        from app.config.database import SessionLocal
        db = SessionLocal()
        try:
            todo = self.repo.find_by_id(db, todo_id, user_id)
            if not todo: return
    ...
    def get_all_todos(self, db: Session, user_id: int) -> list[Todo]:
        return self.repo.find_all(db, user_id)

    def update_todo(self, db: Session, todo_id: int, todo_data: TodoCreate, user_id: int):
        existing_todo = self.repo.find_by_id(db, todo_id, user_id)
    ...
    def delete_todos(self, db: Session, ids: list[int], user_id: int) -> int:
        count = 0
        for todo_id in ids:
            if self.delete_todo(db, todo_id, user_id):
                count += 1
        return count

    def get_todo_by_id(self, db: Session, todo_id: int, user_id: int) -> Todo | None:
        return self.repo.find_by_id(db, todo_id, user_id)
